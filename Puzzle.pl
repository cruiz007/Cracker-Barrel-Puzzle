...
% Dr. Tauru
% CSCE 4430
% This program plays the cracker barrel game using the lambda library. 
% the module lambda library would have to be called before running the progrram
% Before running program run command pack_install(lambda)
...
% implement moves, solution
%
display(Sol) :-
    display(Sol, [1]).
 
 % solution is displayed just like the puzzle
...
display([], Free) :-
    ...
    writeln(We win!).
% output of solution with its moves
